 ...
 * @author Haifeng Li
 */
public class RandomForestSerializationTest {
    long[] seeds = {
    ...
    public RandomForestSerializationTest() {
    }
    ...
    public void testLongley() {
        ...
        RandomForest model = RandomForest.fit(Longley.formula, Longley.data, 100, 3, 20, 10, 3, 1.0, Optional.of(Arrays.stream(seeds)));

        double[] importance = serializeDeserialize(model).importance();
        ...
        double[] prediction = LOOCV.regression(
        		Longley.formula,
        		Longley.data, (f, x) -> serializeDeserialize(RandomForest.fit(f, x, 100, 3, 20, 10, 3, 1.0, Optional.of(Arrays.stream(seeds)))));
    ...
    public void test(String name, Formula formula, DataFrame data, double expected) {
        ...
        MathEx.setSeed(19650218); // to get repeatable results for cross validation.
        double[] prediction = CrossValidation.regression(
        		3,
        		formula,
        		data,
        		(f, x) -> serializeDeserialize(RandomForest.fit(f, x, 100, 3, 20, 100, 5, 1.0, Optional.of(Arrays.stream(seeds)))));
        ...
        RandomForest model = serializeDeserialize(RandomForest.fit(formula, data));
    ...
    @Test
    public void testRandomForestSerializationWithMerging() throws Exception {
        System.out.println("Random forest merging");

        RandomForest forest1 = serializeDeserialize(RandomForest.fit(Abalone.formula, Abalone.train, 50, 3, 20, 100, 5, 1.0, Optional.of(Arrays.stream(seeds))));
        RandomForest forest2 = serializeDeserialize(RandomForest.fit(Abalone.formula, Abalone.train, 50, 3, 20, 100, 5, 1.0, Optional.of(Arrays.stream(seeds).skip(50))));
        RandomForest forest = serializeDeserialize(forest1.merge(forest2));
    ...
    private RandomForest serializeDeserialize(RandomForest model) {
